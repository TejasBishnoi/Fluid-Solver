...
def apply_naca_airfoil(fluid, naca_code, chord_length, center_x, center_y, angle_of_attack_deg=0.0):
    """
    ...
    if angle_of_attack_deg != 0.0:
        theta = np.deg2rad(angle_of_attack_deg)
        sin_theta = np.sin(theta)
        cos_theta = np.cos(theta)
        centered_x = polygon[:, 0] - center_x
        centered_y = polygon[:, 1] - center_y
        rotated_x = centered_x * cos_theta - centered_y * sin_theta
        rotated_y = centered_x * sin_theta + centered_y * cos_theta
        polygon[:, 0] = rotated_x + center_x
        polygon[:, 1] = rotated_y + center_y
